...
export function RegisterMovements({ setState, state, dataSelect, accion }) {
  const { accountItemSelect } = useAccountStore();
  const { dataCategoria, categoriaItemSelect, selectCategoria } = useCategoriesStore(); //------------------
  const { type } = useOperations();
  const { insertarMovimientos } = useMovementsStore(); //------------------
  ...
  const insertar = async (data) => {
    ...
    const p = {
      ...
      worth: parseFloat(data.monto),
      idCategories: categoriaItemSelect.id,
    };
